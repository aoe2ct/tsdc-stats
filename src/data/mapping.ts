...
export const acceptableMisnamedMaps: Record<string, string> = {
  ...
  "[ALS15] Wellsprings - SD2": "TSDC Wellsprings",
...
export const GameNameMappingToDisplayName: Record<string, string> =
  Object.fromEntries(
    Object.values(mapDraftNameToGameNameMapping).map((gameName) => [
      gameName,
      mapNameToDisplay(gameName),
    ]),
  );
...
export const bracketColors: {
  [bracket in keyof typeof BracketNameToImage]: string;
} = {
  Champions: "#1f77b4",
  Monks: "#ff7f0e",
  Mangonels: "#2ca02c",
...
export const winnerStages: string[] = [
  "Group",
  "Quarter Final",
  "Semi Final",
  "Final",
...
function mapNameToDisplay(gameName: string) {
  ...
  if (
    !Object.keys(mapDraftNameToGameNameMapping).find(
      (name) => name.toLowerCase() == sanitizedName.toLowerCase(),
    )
  ) {
    return undefined;
  }
...
export function mapDraftNameToDisplay(draftName: string) {
  return (
    mapNameToDisplay(mapDraftNameToGameNameMapping[draftName] ?? draftName) ??
    draftName
  );
}
